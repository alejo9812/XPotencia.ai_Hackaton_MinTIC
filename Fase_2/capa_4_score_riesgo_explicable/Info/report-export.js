...
export function buildMarkdownReport(report) {
  ...
  lines.push("");
  lines.push(report.traceability?.detail || "Trazabilidad operativa disponible en el informe.");
  lines.push("");
  lines.push(`- **Estado:** ${report.traceability?.statusLabel || "Verificada"}`);
  lines.push(`- **Detalle:** ${report.traceability?.statusDetail || "Origen, modo y cobertura visibles en el informe."}`);
  lines.push("");
  ...
  lines.push("");
  lines.push(`- **Prioridad:** ${report.reviewPriority.label}`);
  lines.push(`- **Lectura:** ${report.reviewPriority.detail}`);
  lines.push(`- **Accion sugerida:** ${report.reviewPriority.action}`);
  lines.push("");
  lines.push("## Plan de revision");
  lines.push("");
  for (const item of report.actionPlan) {
    lines.push(`- ${item}`);
  }
  lines.push("");
  lines.push("## Se̱ales prioritarias");
  lines.push("");
  if (report.signalHighlights.length) {
    for (const item of report.signalHighlights) {
      lines.push(`- ${item}`);
    }
  } else {
    lines.push("- No hay se̱ales prioritarias para destacar.");
  }
  lines.push("");
...
export function buildHtmlReport(report) {
  ...
  const reviewPlan = report.actionPlan.map((item) => `<li>${escapeHtml(item)}</li>`).join("");
  const signalHighlights = report.signalHighlights.length
    ? report.signalHighlights.map((item) => `<li>${escapeHtml(item)}</li>`).join("")
    : "<li>No hay se̱ales prioritarias para destacar.</li>";
  const traceabilityFacts = (report.traceability?.items || [])
    .map(
      (item) => `
        <div class="fact">
          <span>${escapeHtml(item.label)}</span>
  ...
  const reviewPriority = `
    <section class="section review-strip report-wide">
      <div class="review-strip-copy">
        <div class="eyebrow">Lectura ejecutiva</div>
        <h2>${escapeHtml(report.reviewPriority.label)}</h2>
        <p>${escapeHtml(report.reviewPriority.detail)}</p>
        <p class="review-strip-action">${escapeHtml(report.reviewPriority.action)}</p>
      </div>
      <div class="review-strip-score">
        <span>Puntaje</span>
        <strong>${escapeHtml(report.scoreLabel || "--")}</strong>
        <small>${escapeHtml(report.riskLabel || "Sin lectura")}</small>
      </div>
    </section>
  `;
  const traceabilitySection = `
        <article class="section report-wide">
          <h2>Trazabilidad</h2>
          <p>${escapeHtml(report.traceability?.detail || "Trazabilidad operativa disponible en el informe.")}</p>
          <div class="facts-grid">
            ${traceabilityFacts}
          </div>
          <div class="callout">
            <strong>${escapeHtml(report.traceability?.statusLabel || "Verificada")}</strong>
            <p>${escapeHtml(report.traceability?.statusDetail || "Origen, modo y cobertura visibles en el informe.")}</p>
          </div>
        </article>
  `;
...
<html lang="es">
  <head>
    ...
    <style>
      ...
      .review-strip-score span {
        color: #5f7483;
        font-size: 11px;
        text-transform: uppercase;
        letter-spacing: 0.08em;
      }
      .review-strip-score strong {
        font-size: 26px;
        color: #0f172a;
      }
      .review-strip-score small {
        color: #175e53;
        font-weight: 600;
      }
      .section h2 {
  ...
  <body>
    <main class="page">
      ...
      <section class="content">
        ...
        <article class="section">
          <h2>Se̱ales prioritarias</h2>
          <ul>${signalHighlights}</ul>
        </article>
