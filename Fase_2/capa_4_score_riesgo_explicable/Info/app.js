...
function renderExecutivePanel(selected, indicator, source, summary, chartSeries) {
  ...
  const report = buildPdfReportModel({
    ...
    traceability: buildTraceabilityContext(source, summary),
  });
  ...
  dom.executiveSummary.innerHTML = `
    <article class="pdf-report">
      ...
      <section class="report-brief report-callout">
        <div class="report-brief-copy">
          <span class="section-kicker">Lectura ejecutiva</span>
          <h4>${escapeHtml(report.reviewPriority.label)}</h4>
          <p>${escapeHtml(report.reviewPriority.detail)}</p>
        </div>
        <div class="report-brief-badge">
          <span>Puntaje</span>
          <strong>${escapeHtml(report.scoreLabel)}</strong>
          <small>${escapeHtml(report.riskLabel)}</small>
        </div>
      </section>
      ...
      <section class="pdf-report-section pdf-report-wide">
        <h4>Trazabilidad</h4>
        <p>${escapeHtml(report.traceability.detail)}</p>
        <div class="report-facts">
          ${report.traceability.items
            .map(
              (item) => `
        ...
        <div class="report-callout">
          <strong>${escapeHtml(report.traceability.statusLabel)}</strong>
          <p>${escapeHtml(report.traceability.statusDetail)}</p>
        </div>
      </section>
      ...
      <div class="pdf-report-grid">
        ...
        <section class="pdf-report-section">
          <h4>Se̱ales prioritarias</h4>
          <ul class="report-list">
            ${
              report.signalHighlights.length
                ? report.signalHighlights
                    .map((item) => `<li>${escapeHtml(item)}</li>`)
                    .join("")
                : "<li>No hay se̱ales prioritarias para destacar.</li>"
            }
          </ul>
        </section>
...
function buildTraceabilityContext(source, summary) {
  const modeLabel = buildModeLabel();
  const liveSource = state.dataMode === "live" && Boolean(source?.jsonUrl);
  const originLabel = liveSource ? "Web SECOP II" : "Local procesado";

  return {
    label: "Trazabilidad operativa",
    detail: `Fuente ${source.label} con ${formatNumber(summary.totalRecords)} registros y modo ${modeLabel}.`,
    statusLabel: liveSource ? "Sincronizacion web" : "Trazabilidad local",
    statusDetail: liveSource
      ? `Consulta remota activa sobre ${source.datasetId || "dataset"} y ultimo recalculo ${formatTimestamp(state.lastUpdated)}.`
      : `Datos locales procesados para ${source.datasetId || "dataset"} y ultimo recalculo ${formatTimestamp(state.lastUpdated)}.`,
    items: [
      { label: "Fuente", value: source.label },
      { label: "Dataset", value: source.datasetId || "multi-source" },
      { label: "Origen", value: originLabel },
      { label: "Modo", value: modeLabel },
      { label: "Registros", value: formatNumber(summary.totalRecords) },
      { label: "Cobertura", value: `${Math.round(summary.completeness)}%` },
      { label: "Actualizado", value: formatTimestamp(state.lastUpdated) },
    ],
  };
}
...
async function copySummaryToClipboard() {
  ...
  const traceability = buildTraceabilityContext(
    source,
    state.summary || {
      totalRecords: state.visibleRecords.length,
      averageRisk: 0,
      completeness: 0,
      highRiskCount: 0,
      topSignal: "Sin se̱ales dominantes",
    },
  );
  ...
  const summary = [
    ...
    "",
    "Trazabilidad:",
    `- Estado: ${traceability.statusLabel}`,
    `- Detalle: ${traceability.detail}`,
    ...traceability.items.map((item) => `- ${item.label}: ${item.value}`),
    "",
